...
Item {
    ...
    property string current_algorithm2_pic_path: ""//当前算法二图片的路径
    property string current_algorithm3_pic_path: ""//当前算法三图片的路径
    ...
    onVisibleChanged: {
        if(function2.visible == true){
            ...
            closeRadioButton();
            //读取当前索引号
            current_pic_index = operator_user.readIndexFile(starting_ui.user_input_name.text,2);
            //设置xml文件中的索引号
            xml_operation.setIndex(current_pic_index);
            //显示图片总数
            pic_count = xml_operation.getNodeCountsOfXML("file:///D:/色盲数据整理文件/combine_xml_result/step1_select_result_combine.xml");
            //console.log("pic_count = "+pic_count);
            //显示图片
            //获得图片地址
            current_original_pic_path = xml_operation.getOriginOfIndexFromXML("file:///D:/色盲数据整理文件/combine_xml_result/step1_select_result_combine.xml",current_pic_index);
            console.log("hahacurrent_original_pic_path = "+current_original_pic_path);
            console.log("current_pic_index"+current_pic_index);
            ...
            current_algorithm2_pic_path = operator_user.isExist_Algorithm2Pic(original_image.source);
            if(current_algorithm2_pic_path!= "inexistence"){
                algorithm2_fill_image.source = "file:///"+current_algorithm2_pic_path;
                algorithm2_pic_is_exist = 1;
                //显示算法2的按钮
                openRadioButton2();
            }else{
                algorithm2_fill_image.source = "../IMAGE/forground.jpg";
                algorithm2_pic_is_exist = 0;
            }
            //判断是否有对应的算法三填充图，有的话就显示，没有的话用默认图片
            current_algorithm3_pic_path = operator_user.isExist_Algorithm3Pic(original_image.source);
            if(current_algorithm3_pic_path!="inexistence"){
                algorithm3_fill_image.source = "file:///"+current_algorithm3_pic_path;
                algorithm3_pic_is_exist = 1;
                //显示算法3的按钮
                openRadioButton3();
            }else{
                algorithm3_fill_image.source = "../IMAGE/forground.jpg";
    ...
    function getNextGroupPic(){
        ...
        //获得图片地址
        current_original_pic_path = xml_operation.getOriginOfIndexFromXML("file:///D:/色盲数据整理文件/combine_xml_result/step1_select_result_combine.xml",current_pic_index);
        if(current_original_pic_path != "step2_xml_is_end"){
            //xml_operation.getNextNodeData();
            ...
            //将当前的图片索引号存入索引文件中
            operator_user.writeIndexFile(starting_ui.user_input_name.text,current_pic_index,2);
            //关闭按钮使能
            defaultFourRadioButton();
        }else{
            current_pic_index -= 1;
            //将标志位置位回去
            xml_operation.defaultstep2_xml_is_end();
        }
    ...
    Button{
        ...
        onClicked: {
            if(current_pic_index>0){
                ...
                original_image.source = current_original_pic_path;
                //判断是否有对应的算法二填充图，有的话就显示，没有的话用默认图片
                current_algorithm2_pic_path = operator_user.isExist_Algorithm2Pic(original_image.source);
                if(current_algorithm2_pic_path!= "inexistence"){
                    algorithm2_fill_image.source = "file:///"+current_algorithm2_pic_path;
                }else{
    ...
    Button{
        ...
        onClicked: {
            if(current_pic_index<pic_count-1){
                ...
                if(current_algorithm2_pic_path!= "inexistence"){
                    algorithm2_fill_image.source = "file:///"+current_algorithm2_pic_path;
                    //console.log("algorithm2_fill_image.source="+algorithm2_fill_image.source);
                }else{
                    algorithm2_fill_image.source = "../IMAGE/forground.jpg";
                }
                //判断是否有对应的算法三填充图，有的话就显示，没有的话用默认图片
